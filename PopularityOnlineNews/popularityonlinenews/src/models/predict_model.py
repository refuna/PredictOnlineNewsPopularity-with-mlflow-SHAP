...
@click.argument('predictions_dir', type=click.Path())
def main(test_filepath, models_dir, predictions_dir):
    ...
    # save predict model
    # if not os.path.exists(models_dir + '/rfp.joblib'):
    #     joblib.dump(rf, models_dir + '/rfp.joblib')
    #     logger.info('PredictModel successfully saved')

    if 'popularity' in data.columns:
        # rmse = mean_squared_error(y, y_pred, squared=False)
        # mse = mean_squared_error(y, y_pred, squared=True)
        # logger.info(f"Model successfully evaluated. RMSE: {rmse}, MSE: {mse}")
    ...
    # save predictions
    predictions = pd.DataFrame(y_pred, columns=['popularity'])
